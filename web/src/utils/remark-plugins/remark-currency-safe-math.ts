...
/** Share the same math grammar with the rich editor's lossless Markdown parser. */
export function currencySafeMathOptions(): Options {
  const data: { micromarkExtensions?: Options["extensions"]; fromMarkdownExtensions?: Options["mdastExtensions"] } = {};
  remarkCurrencySafeMath.call({ data: () => data });
  return { extensions: data.micromarkExtensions, mdastExtensions: data.fromMarkdownExtensions };
}
