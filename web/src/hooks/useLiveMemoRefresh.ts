...
export function useLiveMemoRefresh() {
  ...
  useEffect(() => {
    ...
    const channel = currentUserName ? createSSESyncChannel(currentUserName) : null;
...
function createSSESyncChannel(userName: string): BroadcastChannel | null {
  ...
  try {
    return new BroadcastChannel(`${SSE_SYNC_CHANNEL_NAME}:${userName}`);
  } catch {
    return null;
  }
}
...
async function runWithSSELeadership(
  signal: AbortSignal,
  task: () => Promise<void>,
  coordinateAcrossTabs: boolean,
  userName: string,
): Promise<void> {
  ...
  await lockManager.request(`${SSE_CONNECTION_LOCK_NAME}:${userName}`, { mode: "exclusive", signal }, async (lock) => {
