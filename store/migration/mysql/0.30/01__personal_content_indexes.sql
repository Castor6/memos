...
ALTER TABLE memo ADD COLUMN space VARCHAR(256) COLLATE utf8mb4_bin NOT NULL DEFAULT '';
ALTER TABLE memo ADD COLUMN is_todo BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE attachment ADD COLUMN space VARCHAR(256) COLLATE utf8mb4_bin NOT NULL DEFAULT '';

UPDATE memo SET space = COALESCE(JSON_UNQUOTE(JSON_EXTRACT(payload, '$.space')), ''), is_todo = COALESCE(JSON_UNQUOTE(JSON_EXTRACT(payload, '$.isTodo')), 'false') = 'true', payload = JSON_REMOVE(payload, '$.space', '$.isTodo');
UPDATE attachment SET space = COALESCE(JSON_UNQUOTE(JSON_EXTRACT(payload, '$.space')), ''), payload = JSON_REMOVE(payload, '$.space');
