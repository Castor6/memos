...
- 实际验证：根测试 13 项、发布脚本测试 29 项通过；打包测试 23 项通过、1 项因本机文件系统大小写规则跳过；Actionlint 通过。扩展 lint、426 项测试、类型/语言表检查及生产构建通过。

补充完整本地模拟（2026-09-23）：在一次性 detached worktree 执行真实 Changesets version、生成提交、精确再生成校验、工作流发布判断、Vite 构建、ZIP 打包和公开前 prepare。结果：Memos 保持 0.8.2，根 CHANGELOG 字节不变；扩展生成 0.1.0，发布判断为 `memos=false` / `web_clipper=true`，校验后的五个文件为扩展 ZIP、CHANGELOG、LICENSE、release.json、SHA256SUMS。没有调用 GitHub 上传或镜像操作。该模拟发现并修复临时依赖 symlink 被版本校验误认作生成文件的问题，测试夹具现按实际 `node_modules/` 忽略规则覆盖。
