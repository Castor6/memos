...
- **Pick up** 用于自己的 X 发帖、回复或引用帖详情，自动取得当前帖与页面已加载的必要上文。保存「Pick up → Context & thinking（可选）→ What they put down（有内容时）」，保留作者、时间、来源与原始互动快照。本人原话只读，Context & thinking 记录形成发帖之前的背景、线索与思考过程。没有提取到相关上文时按独立发帖整理，省略空章节，不提示上文缺失。明确检测到正文未展开或引用不可用等情况时，仅在扩展中提示，不写入笔记正文。
