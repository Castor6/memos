...
- 「预览完整保存内容」渲染 Markdown 标题、列表、引用、代码、表格、图片与长文折叠；历史正文使用同一预览组件。不可信 HTML 经清理后显示。已保存笔记不会因模板改名被批量改写；旧的未知结果保存请求仍按原正文重试。
...
转存失败的图片保留原链接，文字继续保存，弹窗可展开查看失败地址和原因。外链能否继续显示取决于原站。每次最多转存 100 个不同来源，单图最大 10 MiB、下载超时 8 秒；只转存可安全处理的 HTTPS/内嵌位图，不发送原站登录凭据，不跟随下载重定向。未支持或超限图片同样保留链接并提示。保存大小先按转存后预计正文检查，最终仍按实际正文校验服务器限制。
...
右键「快速保存选区/图片」同样将图片转存并放入正文，仍不包含思考编辑或结构化记录同步，也不具备弹窗的持久化操作恢复。旧本地历史不自动迁移成云记录，原有笔记仍留在 Memos。回滚到不认识 `capture` 的旧服务端后编辑笔记可能丢失新快照字段，回滚前应保留数据库备份。
...
继续维护关联任务中的实际验证与未覆盖范围。扩展修改触发独立 lint、单测、Python 打包测试、构建和 ZIP 生成，纳入根仓库 `validate`；版本 PR 与工作流修改也运行该检查。扩展交付行为变化需在根目录运行 `corepack pnpm changeset`，同时选择 `memos-web-clipper` 与 `memos-personal`：前者决定扩展升级级别，后者记录配套交付并触发现有统一 Release；仅应用变化只选择 `memos-personal`。机器人分别生成根目录与 `release/` 下的版本和 CHANGELOG，普通 PR 不手改。不要把检查通过当作扩展商店发布或服务器部署完成。新增界面文案以个人使用的中文为主，上游设置与通用文案仍沿用语言配置。
