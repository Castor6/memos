...
describe("isSupportedVersion", () => {
  it("accepts only the audited Castor fork release below the upstream floor", () => {
    expect(isSupportedVersion("0.7.0")).toBe(true);
    expect(isSupportedVersion("v0.7.0")).toBe(true);
    expect(isSupportedVersion("0.7.1")).toBe(false);
    expect(isSupportedVersion("0.8.0")).toBe(false);
    expect(isSupportedVersion("0.7.0-beta.1")).toBe(false);
