...
- Add a new `.changeset/*.md` for shipped behavior changes using root `corepack pnpm changeset`. Use patch/minor/major and a Chinese user-facing summary. 扩展交付变化的 Changeset 同时选择 `memos-web-clipper` 和 `memos-personal`；扩展独立版本/日志位于 `extensions/web-clipper/release/`，仅应用更新不升级扩展。Only the generated version PR updates release package versions and their `CHANGELOG.md`; upstream history lives in `docs/upstream/CHANGELOG.md`. Version PR merges publish smoke-tested ACR images; server pull updates use `scripts/deploy/` with backup and recovery. Keep deployment credentials and host details out of this repository.
