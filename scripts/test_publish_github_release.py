...
class ReleaseWorkflowTests(unittest.TestCase):
    ...
    def test_only_memos_lane_can_access_image_publishing_and_registry_credentials(self):
        ...
        self.assertIn('needs: [source, candidate, registry]', jobs['github-release'])
        self.assertIn('fail-fast: false', jobs['registry'])
        self.assertIn("if: always() && needs.candidate.result == 'success'", jobs['github-release'])
        self.assertNotIn('continue-on-error', jobs['registry'])
        ...
        self.assertIn('python3 .publication-tools/scripts/publish-image.py', jobs['candidate'])
        self.assertIn('ref: ${{ github.workflow_sha }}', jobs['candidate'])
        self.assertIn('statuses: write', jobs['candidate'])
        self.assertIn('actions: read', jobs['candidate'])
        self.assertLess(jobs['candidate'].index('id: candidate-artifact'), jobs['candidate'].index('publish-image.py receipt'))
        self.assertIn('needs: [source, candidate]', jobs['registry'])
        self.assertIn('needs.candidate.outputs.artifact_name', jobs['registry'])
        self.assertIn('name: published-${{ matrix.channel }}\n          overwrite: true', jobs['registry'])
        self.assertLess(jobs['registry'].index('publish-image.py publish'), jobs['registry'].index('name: published-'))
        self.assertNotIn('overwrite: true', jobs['candidate'])
        self.assertIn('overwrite: true', jobs['clipper-candidate'])
