...
func (w *Writer) WriteAttachment(path string, content io.Reader) (string, int64, error) {
	...
	// Binary attachments are stored verbatim so highly compressible files cannot
	// produce an export that violates the reader's expansion-ratio limit.
	entry, err := w.create(path, zip.Store)
	if err != nil {
		return "", 0, err
	}
	digest := sha256.New()
